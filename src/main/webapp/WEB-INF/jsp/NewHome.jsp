<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>ChiTou- 探索熱門景點、獨特體驗、交通、飯店住宿、租車、美食</title>
<%@ include file="/WEB-INF/includes/newhomepage/NewHomeHead.jsp"%>
</head>
<body>
<%@ include file="/WEB-INF/includes/newhomepage/TopBar.jsp"%>
<%@ include file="/WEB-INF/includes/newhomepage/HeroImages.jsp"%>
<%@ include file="/WEB-INF/includes/newhomepage/Row1.jsp"%>
<%@ include file="/WEB-INF/includes/newhomepage/AttractionsRow.jsp"%>
<%@ include file="/WEB-INF/includes/newhomepage/HotelsRow.jsp"%>
<%@ include file="/WEB-INF/includes/newhomepage/Row4.jsp"%>
<%@ include file="/WEB-INF/includes/newhomepage/Footer.jsp"%>
<%-- <%@ include file="/WEB-INF/includes/newhomepage/Loading.jsp"%> --%>


</body>
</html>