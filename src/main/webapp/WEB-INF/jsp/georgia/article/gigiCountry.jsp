...
for (Article tt : list) {
	if(tt.getUserDelete()==1)
		continue;
	if(tt.getManageHidden()==1)
		continue;
...
<script type="text/javascript">
		$(function(){
			...
			$(document).on("click",".btn_comment_overview_m",function(){
				let comment = $(this).closest('td').prev().find('#comment').val();
				console.log("OM"+comment);
	    ...
	    $('#detailReply').append(
                $('<tr/>')
                    ...
                    .append(
                        $('<td/>')
                            .append(
                                $('<input/>').addClass("btn_comment_overview_m")
                                    .attr('type', 'button')
                                    .attr('value', '送出')
                                    .attr('id', 'commentNew')
